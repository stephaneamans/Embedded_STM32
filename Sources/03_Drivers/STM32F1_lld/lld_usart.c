 ...
 *
 */
static t_error_handling usart_enable_clock(struct t_usart_driver *driver)
{
...
t_error_handling usart_init(struct t_usart_driver *driver, struct t_usart_config *config)
{
    ...
    else if(driver->reg == USART2)
    {
    	...
    	clock_frequency = get_apb1_clock();
    	driver->priv = &priv[1];
    	memset(driver->priv, 0, sizeof(struct t_usart_private));
    }
    else
...
struct t_usart_driver *usart_get_driver(uint8_t usart_number)
{
    ...
    if((usart_number > 0)  && (usart_number <= TOTAL_USART_IP))
    {
